Shader "Sprites/SpriteWithExtras"
{
	Properties
	{
		[Header(Texture)]
		...
		[Header(Orientation)]
		[MaterialToggle] _InvertUV("Flip",float) = 0
	...
	SubShader
	{
		...
		void processFadeInEffects(Input i) {
			float uvx = (_FadeInStripsHorizontalReverse ? 1* i.vertex.x * 2 : 0) + (i.uv_MainTex.x) * (_FadeInStripsHorizontalReverse ? -1 : 1);
			float uvy = (_FadeInStripsVerticalReverse ? 1* i.vertex.y * 2 : 0) + i.uv_MainTex.y * (_FadeInStripsVerticalReverse ? -1 : 1);
		...
		void surf(Input IN, inout SurfaceOutput o) {
			...
			// Fade In Effects 
			processFadeInEffects(IN); 

			o.Albedo = col.rgb;  
			o.Alpha = col.a;
			
			float3 n = UnpackNormal(tex);
			//o.Normal = _Flip ? -n : n;    
		} 
		ENDCG
	}
